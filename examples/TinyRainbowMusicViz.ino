...
void loop() {
  if (eq.num_bands > NUM_LEDS) { while(true){} }  // must be this tall to ride
  for (int j = 0; j < eq.num_bands; j++) {
    int spectralReading = eq.readBandAt(j);
    spectralReading = map(spectralReading, 0, 1024, 0, 256);
    CRGB colour = CRGB(0, 0, spectralReading);
    leds[j] = colour;
  }
  FastLED.show();
}
